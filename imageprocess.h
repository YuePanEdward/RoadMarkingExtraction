...
namespace image{
	...
	class Imageprocess
	{
	  public:
		  ...
		  void savepcgrid(vector<double> &boundingbox, float resolution, const pcXYZIPtr &c);   //投影点云格网化并记录点号
		  void savepcgrid(vector<double> &boundingbox, float resolution, const pcXYZIPtr &c, const pcXYZIPtr &gc, const pcXYZIPtr &ngc);   //投影点云格网化并记录点号, 重载，地面和非地面点
		  void pc2imgI(const pcXYZIPtr &cloud, int whatcloud,  Mat &img, float times_std);               //点云转反射强度图像  whatcloud[ 0: Original Cloud , 1: Ground Cloud , 2: Non-ground Cloud]; //times_std: maxI = meanI+ times_std*stdI
		  ...
		  void pc2imgD(const pcXYZIPtr &cloud, int whatcloud,  Mat &img ,float k);      //点云转点密度图像    whatcloud[ 0: Original Cloud , 1: Ground Cloud , 2: Non-ground Cloud] k:expected max point number in a pixel
		 
		  void img2pc_g(const Mat &img, const pcXYZIPtr &incloud, pcXYZIPtr & outcloud);  //标线图像转原始无过滤整体点云 (地面点）
		  void img2pc_c(const Mat &img, const pcXYZIPtr &incloud, pcXYZIPtr & outcloud);  //标线图像转原始无过滤整体点云（所有点）

		  void img2pclabel_g(const Mat &img, const pcXYZIPtr &incloud, vector<pcXYZI> &outclouds, double dZ);   //分割标线图像转分割点云(地面点）
		  void img2pclabel_c(const Mat &img, const pcXYZIPtr &incloud, vector<pcXYZI> &outclouds, double dZ);   //分割标线图像转分割点云（所有点）
		  ...
		  Mat ExtractRoadPixelIZD(const Mat & _binI,const Mat & _binZ,const Mat & _binD); //用高程梯度及点密度滤除非道路像素 (for MLS)
		  Mat ExtractRoadPixelIZ (const Mat & _binI,const Mat & _binZ);                   //用高程梯度滤除非道路像素 (for ALS)
		  ...
		  void Truncate( Mat & Img, Mat & TImg);                                    //图像像素灰度值非零区域截取
		  ...
		  void saveimg(const Mat &ProjI, const Mat &ProjZ, const Mat &ProjD, const Mat &ProjImf, const Mat &GI, const Mat &GZ, const Mat &BZ, const Mat &BD, const Mat &GIR, const Mat &BI, const Mat &BIF, const Mat &Label); // 重载 1
		  void saveimg(const Mat &ProjI, const Mat &ProjImf, const Mat &GI, const Mat &BI, const Mat &BIF, const Mat &Label); //重载 2
